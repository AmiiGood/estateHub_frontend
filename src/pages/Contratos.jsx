  ...
  const confirmarEliminacion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/contratos/putEstatusContrato/${contratoAEliminar}`,
        {
          method: "PUT",
          headers: {
                  ...
                  <div className="p-6 text-gray-100">
                    ...
                    <div className="space-y-2">
                      ...
                      {tieneContratos && (
                        <div className="flex gap-2">
                          <Link
                            to={`/contrato/${contratoActivo?.idContrato ||
                          ...
                          <Link
                            to={`/editarContrato/${contratoActivo?.idContrato ||
                              prop.contratos[0].idContrato
                              }`}
...
export const loaderContratos = async ({ request }) => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/propiedades/getPropiedadesByUsuario/${idUsuario}`, {
      headers,
    });
      ...
      propiedades.map(async (prop) => {
        try {
          const contratosRes = await fetch(
            `${import.meta.env.VITE_API_URL}/contratos/getContratosByPropiedad/${prop.idPropiedad}`,
            { headers }
          );
