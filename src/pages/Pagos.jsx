...
export const loaderPagos = async ({ request }) => {
  ...
  try {
    ...
    const resPropiedades = await fetch(
      `${API_BASE}/propiedades/getPropiedadesByUsuario/${idUsuario}`,
      { headers }
    );
    ...
    const resContratos = await fetch(
      `${API_BASE}/contratos/getContratosByUsuario/${idUsuario}`,
      { headers }
    );
    ...
    if (estatus) {
      const resPagos = await fetch(
        `${API_BASE}/pagosRenta/getPagosRentaByEstatus/${estatus}`,
        { headers }
      );
      ...
      const resPagos = await fetch(
        `${API_BASE}/pagosRenta/getPagosRentaByRangoFechas?inicio=${inicio}&fin=${fin}`,
        { headers }
      );
      const dataPagos = await resPagos.json();
      pagos = dataPagos.data || [];
    } else {
      const resPagos = await fetch(`${API_BASE}/pagosRenta/getPagosRenta`, {
        headers,
      });
      const dataPagos = await resPagos.json();
      pagos = dataPagos.data || [];
